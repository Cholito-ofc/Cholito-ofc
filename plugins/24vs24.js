...
let handler = async (msg, { conn, args }) => {
  ...
  if (!horaTexto) {
    return conn.sendMessage(chatId, { text: "✳️ Usa el comando así:\n*.24vs24 [hora] [modalidad]*\nEjemplo: *.24vs24 5:00pm CLK*" }, { quoted: msg })
  }
...
`.trim()
    ...
    partidasVS24[newMsg.key.id] = data
    partidasVS24[newMsg.key.id].originalMsgKey = newMsg.key
    delete partidasVS24[key.id]
  })
}

handler.command = ['vs24']
