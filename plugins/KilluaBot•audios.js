const handler = async (m, { conn }) => {
  ...
  try {
    ...
    if (audioLinks[text]) {
      await conn.sendMessage(m.chat, { react: { text: '🎧', key: m.key } }); // Reacción tipo Killua
      conn.sendPresenceUpdate('recording', m.chat);
      await conn.sendFile(m.chat, audioLinks[text], `${text}.mp3`, null, m, true, { type: 'audioMessage' });
    }
  } catch (e) {
    console.error(`[❌ ERROR - AUDIOS]`, e);
    m.reply('⚠️ Ocurrió un error al enviar el audio.');
  }
};
