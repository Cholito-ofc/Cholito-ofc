...
const BLOCKED_FILE = './bloqueados.json'; // Archivo de usuarios bloqueados
...
const handler = async (msg, { conn, args, command }) => {
  ...
  if (bloqueados.includes(msg.sender)) return; // Ignorar usuario bloqueado

  if (!args.length) return conn.reply(msg.chat, '❗ ɪɴɢʀᴇsᴀ ᴜɴ ᴛᴇxᴛᴏ', msg);
  ...
  try {
    ...
    await conn.sendMessage(msg.chat, { image: { url: res.imageUrl }, caption: '' }, { quoted: msg });
  } catch (e) {
