const { Maker } = require('imagemaker.js'); // Requiere imagemaker.js si lo tienes ya instalado
const handler = async (m, { conn, args, command }) => {
  ...
  const commands = {
    ...
    logomurcielago: 'https://en.ephoto360.com/create-batman-logo-style-text-effect-online-1080.html',
  ...
  if (commands[command]) {
    try {
      ...
      const res = await new Maker().Ephoto360(commands[command], [response[0]]);
